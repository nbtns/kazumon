...
export function TitleScreen({ onStart }: TitleScreenProps) {
  ...
  return (
    <div className="h-[100dvh] flex flex-col items-center justify-start p-3 2xl:p-6 pt-4 2xl:pt-8 gap-2 2xl:gap-4 overflow-hidden">
      {/* タイトルロゴ */}
      ...
      {/* デモブロック（タイトル画面の飾り） */}
      <div className="flex gap-2 2xl:gap-3 animate-float">
        {demoPrimes.map((prime) => (
          <div
            key={prime}
            className="w-8 h-8 2xl:w-12 2xl:h-12 rounded-lg shadow-md border-2 border-white/50"
            style={{
      ...
      {/* カラーガイド（ブロックの色と素数の対応） */}
      <div className="w-full max-w-md bg-white/70 rounded-2xl p-2 2xl:p-4 shadow-md border-2 border-kazumon-light/30">
        <p className="text-center text-sm 2xl:text-lg font-bold text-kazumon-dark mb-1 2xl:mb-2">
          🎨 ブロックのいろガイド
        </p>
        <div className="grid grid-cols-2 gap-1.5 2xl:gap-3">
          {colorGuide.map((item) => (
            <div
              key={item.prime}
              className="flex items-center gap-2 bg-white rounded-lg p-1 2xl:p-2 shadow-sm"
            >
              {/* 色見本 */}
              <div
                className="w-5 h-5 2xl:w-8 2xl:h-8 rounded-md shadow-sm border-2 border-white/50 flex-shrink-0"
                style={{ backgroundColor: PRIME_COLORS[item.prime] }}
              />
              {/* 数字・色名・説明 */}
              <div className="flex flex-col leading-tight">
                <span className="text-xs 2xl:text-base font-bold text-kazumon-dark">
                  {item.prime}（{item.colorName}）
                </span>
                <span className="text-[10px] 2xl:text-sm text-kazumon-dark/70">
                  {item.description}
                </span>
              </div>
            </div>
      ...
      {/* 難易度選択 */}
      <div className="flex flex-col gap-2 2xl:gap-3 w-full max-w-md">
        <p className="text-center text-lg 2xl:text-2xl font-bold text-kazumon-dark">
          レベルをえらんでね！
        </p>
        {difficulties.map((diff) => {
          ...
          return (
            <button
              ...
              `}
            >
              <span className="text-xl 2xl:text-3xl">{config.label}</span>
              <span className="text-xs 2xl:text-base opacity-90">{config.description}</span>
              {/* ベストタイム（記録がある場合のみ表示） */}
              {hasRecord && best && (
                <span className="text-xs 2xl:text-sm bg-white/30 rounded-full px-2 py-0.5 mt-0.5 tabular-nums">
